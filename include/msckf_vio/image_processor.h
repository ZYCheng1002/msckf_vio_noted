...
/**
 * @brief ImageProcessor
 * Detects and tracks features in image sequences.
 */
class ImageProcessor {
 ...
 private:
  /**
   * @brief ProcessorConfig
   * Configuration parameters for feature detection and tracking.
   */
   ...
   * Initialize the image processing sequence, which is bascially detect new features on the first set of stereo images.
   */
  ...
  /**
   * @brief addNewFeatures
   * Detect new features on the image to ensure that the features are uniformly distributed on the
   * image.
   */
  void addNewFeatures();
   ...
   * Remove some of the features of a grid in case there are too many features inside of that grid, which ensures the
  ...
  /**
   * @brief publish
   * Publish the features on the current image including both the tracked and newly detected ones.
   */
  void publish();
  ...
  /**
   * @brief integrateImuData
   * Integrates the IMU gyro readings between the two consecutive images, which is used for both tracking prediction and
   * 2-point RANSAC.
  ...
  /**
   * @brief predictFeatureTracking
   * Compensates the rotation between consecutive camera frames so that feature tracking would be more robust and fast.
   ...
   *
   * @note the input and output points are of pixel coordinates.
   */
   ...
   * @param cam0_points: 主图像上的点
   * @return cam1_points: 另外个相机的点
   * @return inlier_markers: 合法点为1,非法点为0
   */
  ...
  std::string cam0_distortion_model;  // 相机0畸变模型
  cv::Vec2i cam0_resolution;          // 相机0尺寸
  cv::Vec4d cam0_intrinsics;          // 相机0畸变参数
  cv::Vec4d cam0_distortion_coeffs;   // 相机0畸变参数

  std::string cam1_distortion_model;  // 相机0畸变模型
  cv::Vec2i cam1_resolution;          // 相机0尺寸
  cv::Vec4d cam1_intrinsics;          // 相机0畸变参数
  cv::Vec4d cam1_distortion_coeffs;   // 相机0畸变参数
  ...
  boost::shared_ptr<GridFeatures> prev_features_ptr;  // 上一帧特征
  boost::shared_ptr<GridFeatures> curr_features_ptr;  // 当前帧特征
