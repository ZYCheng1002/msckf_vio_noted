...
bool ImageProcessor::loadParameters() {
  ...
  ROS_INFO(
      "cam0_intrinscs: %f, %f, %f, %f", cam0_intrinsics[0], cam0_intrinsics[1], cam0_intrinsics[2], cam0_intrinsics[3]);
  ROS_INFO("cam0_distortion_model: %s", cam0_distortion_model.c_str());
  ROS_INFO("cam0_distortion_coefficients: %f, %f, %f, %f",
           cam0_distortion_coeffs[0],
           cam0_distortion_coeffs[1],
           cam0_distortion_coeffs[2],
           cam0_distortion_coeffs[3]);

  ROS_INFO("cam1_resolution: %d, %d", cam1_resolution[0], cam1_resolution[1]);
  ROS_INFO(
      "cam1_intrinscs: %f, %f, %f, %f", cam1_intrinsics[0], cam1_intrinsics[1], cam1_intrinsics[2], cam1_intrinsics[3]);
  ROS_INFO("cam1_distortion_model: %s", cam1_distortion_model.c_str());
  ROS_INFO("cam1_distortion_coefficients: %f, %f, %f, %f",
           cam1_distortion_coeffs[0],
           cam1_distortion_coeffs[1],
           cam1_distortion_coeffs[2],
           cam1_distortion_coeffs[3]);
...
void ImageProcessor::createImagePyramids() {
  ...
  buildOpticalFlowPyramid(curr_cam0_img,
                          curr_cam0_pyramid_,
  ...
  buildOpticalFlowPyramid(curr_cam1_img,
                          curr_cam1_pyramid_,
                          Size(processor_config.patch_size, processor_config.patch_size),
                          processor_config.pyramid_levels,
                          true,
...
void ImageProcessor::trackFeatures() {
  ...
  calcOpticalFlowPyrLK(
      ...
      processor_config.pyramid_levels,
      TermCriteria(
          TermCriteria::COUNT + TermCriteria::EPS, processor_config.max_iteration, processor_config.track_precision),
  ...
  twoPointRansac(prev_matched_cam0_points,
  ...
  twoPointRansac(prev_matched_cam1_points,
...
void ImageProcessor::stereoMatch(const vector<cv::Point2f>& cam0_points,
  ...
  /// 光流跟踪, 将跟踪上的点对保存到inline_markers中
  calcOpticalFlowPyrLK(
      curr_cam0_pyramid_,
      curr_cam1_pyramid_,
      cam0_points,
  ...
  const cv::Matx33d t_cam0_cam1_hat(
      0.0, -t_cam0_cam1[2], t_cam0_cam1[1], t_cam0_cam1[2], 0.0, -t_cam0_cam1[0], -t_cam0_cam1[1], t_cam0_cam1[0], 0.0);
...
void ImageProcessor::undistortPoints(const vector<cv::Point2f>& pts_in,
  ...
  const cv::Matx33d K_new(
      new_intrinsics[0], 0.0, new_intrinsics[2], 0.0, new_intrinsics[1], new_intrinsics[3], 0.0, 0.0, 1.0);
...
void ImageProcessor::publish() {
  ...
  undistortPoints(
      curr_cam0_points, cam0_intrinsics, cam0_distortion_model, cam0_distortion_coeffs, curr_cam0_points_undistorted);
  undistortPoints(
      curr_cam1_points, cam1_intrinsics, cam1_distortion_model, cam1_distortion_coeffs, curr_cam1_points_undistorted);
...
void ImageProcessor::drawFeaturesStereo() {
  ...
  /// Colors for different features.
  Scalar tracked(0, 255, 0);        // 追踪的特征颜色
  Scalar new_feature(0, 255, 255);  // 新识别的特征颜色
  ...
  int img_height = cam0_curr_img_ptr->image.rows;
  int img_width = cam0_curr_img_ptr->image.cols;
  Mat out_img(img_height, img_width * 2, CV_8UC3);  // 可视化双目,所以width x 2
  /// 投影原图为彩色
